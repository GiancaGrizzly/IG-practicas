...
void _X_revolution_object::generate_vertex()
{
    ...
    //Reservo espacio para el vector de vértices de la zona central y para el vector de
    //vértices final, en función de si hay o no tapaderas
    ...
    else if (lower) {           //Si el objeto solo tiene lower cover
    ...
    else if (upper) {           //Si el objeto solo tiene upper cover
        Central.resize(Profile.size()-1);
        for (unsigned int i=0; i < Central.size(); i++)
            Central[i] = Profile[i];
    ...
    for (unsigned int i=0; i < N; i++) { //N rotaciones
        ...
        for (unsigned int j=0; j < Central.size(); j++) { //calculo las coordenadas de cada vertice de la zona central
            Vertices[Central.size()*i+j].x =  Central[j].x * COS;
            Vertices[Central.size()*i+j].y =  Central[j].y;
            Vertices[Central.size()*i+j].z = -Central[j].x * SIN;
        }
    }
}
...
{
    //Reservo espacio para el vector de triángulos y los calculo, en función de si hay o no tapaderas
    if (lower and upper) {  //Si el objeto tiene tapadera inferior y superior
        Triangles.resize(2*N*(Central.size()-1) + 2*N);
        unsigned int mod = Vertices.size()-2;   //Variable módulo para la operación %
        ...
        unsigned int mod = Vertices.size()-1;   //Variable módulo para la operación %
